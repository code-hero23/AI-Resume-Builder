...
const Dashboard = () => {
  ...
  const [showUploadResume, setShowUploladResume] = useState(false);
  ...
  const createResume = async(event) => {
    ...
    navigate('/app/builder/resume2313')
  }
  const uploadResume = async(event)=>{
 event.preventDefault()
    setShowUploladResume(false)
    navigate('/app/builder/resume2313')
  }
    ...
    <div>
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-4">
          ...
          <button onClick={()=>setShowUploladResume(true)} className="w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed border-slate-300 group hover:border-orange-500 hover:shadow-lg transition-all duration-300 cursor-pointer">
            <UploadIcon className="size-11 transition-all duration-300 p-2.5 bg-gradient-to-br from-orange-300 to-orange-500 text-white rounded-full" />
            <p className="text-sm group-hover:text-orange-600 transition-all duration-300">
        ...
        <div className="grid grid-cols-2 sm:flex flex-wrap gap-4">
          {allResumes.map((resume, index) => {
            ...
            return (
              ...
              >
                <FilePenLineIcon // <-- This needs to be imported
          ...
          showCreateResume &&(
            <form onSubmit={createResume} onClick={()=>setShowCreateResume(false)} className="fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center">
              <div onClick={e=>e.stopPropagation()} className="relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6">
                ...
                <input onChange={(e)=>setTitle(e.target.value)} value={title} type="text" placeholder="Enter Resume Title"  className="w-full px-4 py-2 focus:border-orange-600 ring-orange-600 mb-2" required />
        ...
        }
        {
          showUploadResume &&(
            <form onSubmit={uploadResume} onClick={()=>setShowUploadResume(false)} className="fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center">
              ...
              <button className="w-full py-2 bg-orange-600 text-white rounded hover:bg-orange-700 transition-colors">Upload Resume</button>
              <XIcon className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 cursor-pointer transition-colors" onClick={()=>{
                setShowUploladResume(false); setTitle('')
              }}/>
              </div>
            </form>
          )
